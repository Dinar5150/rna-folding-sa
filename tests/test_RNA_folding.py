...
import RNA_folding
from dwave.samplers import SimulatedAnnealingSampler  # use simulated annealing

# (Comment out the LeapHybridCQMSampler import since we are no longer using it)
...
class TestRNA_folding(unittest.TestCase):
    ...
    def test_build_cqm(self):
        ...
        stem_dict = {
            (1, 3, 13, 15): [(1, 3, 13, 15)],
            (6, 10, 20, 24): [(6, 8, 22, 24), (6, 9, 21, 24), (6, 10, 20, 24),
                              (7, 9, 21, 23), (7, 10, 20, 23), (8, 10, 20, 22)],
            (7, 9, 14, 16): [(7, 9, 14, 16)],
        ...
        self.assertEqual(cqm.variables,
                         [(1, 3, 13, 15), (6, 8, 22, 24), (6, 9, 21, 24), (6, 10, 20, 24),
                          (7, 9, 21, 23), (7, 10, 20, 23), (8, 10, 20, 22),
                          (7, 9, 14, 16), (13, 15, 23, 25), 'Null:(6, 10, 20, 24)'])
        target_linear = {(1, 3, 13, 15): -9.0, (6, 8, 22, 24): -9.0, (6, 9, 21, 24): -16.0,
                         (6, 10, 20, 24): -25.0, (7, 9, 21, 23): -9.0, (7, 10, 20, 23): -16.0,
                         (8, 10, 20, 22): -9.0, (7, 9, 14, 16): -9.0, (13, 15, 23, 25): -9.0,
                         'Null:(6, 10, 20, 24)': 0.0}
        ...
        self.assertEqual(cqm.objective.quadratic,
                         {((6, 8, 22, 24), (1, 3, 13, 15)): 2.6999999999999997,
                          ((6, 9, 21, 24), (1, 3, 13, 15)): 3.5999999999999996,
                          ((6, 10, 20, 24), (1, 3, 13, 15)): 4.5,
    ...
    def test_small_case(self):
        ...
        # Convert the CQM to an equivalent BQM using a penalty to enforce constraints
        bqm, _ = cqm.to_bqm(penalty=10)
        sampler = SimulatedAnnealingSampler()
        sample_set = sampler.sample(bqm, num_reads=100)

        # Note: RNA_folding.process_cqm_solution may need adjustment if it was tailored for CQM results.
